...
describe("Obsidian MCP Server E2E Tests", () => {
	...
	test(
		"vault의 read 액션은 적절하게 문서를 읽어올 수 있는가?",
		async () => {
	...
	test(
		"list_all 도구는 vault의 모든 문서 목록을 반환한다",
		async () => {
	...
	test(
		"vault의 collect_context 액션은 배치 메모리 패킷을 반환한다",
		async () => {
			let response: CompatibilityCallToolResult | undefined;
	...
	test(
		'search 도구는 "Test Note" 키워드를 기반으로 문서를 찾을 수 있다',
		async () => {
		...
		async () => {
			...
			for (let i = 0; i < maxRetries; i++) {
				...
				if (!response.isError) {
					...
					// 에러 내용 로깅 (디버깅 용도)
					if (i % 10 === 0) {
						const errorText = (response.content as { type: string; text: string }[])
