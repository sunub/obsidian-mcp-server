...
권장 쓰임:
- 1차 후보 확인
- Agent Context Pipeline에 넣을 문서/청크 후보 확인
- 개념 키워드 또는 의미적으로 가까운 문서가 맞는지 필터링
...
목적: 주제/전체 문맥을 배치 단위로 모아 Agent가 사용할 수 있는 `memory_packet` 형태로 선별/압축.
