...
function formatAsContext(payload: any): string {
	...
	if (memory_packet.keyFacts?.length > 0) {
		contextParts.push("  <key_facts>");
		memory_packet.keyFacts.forEach((fact: string) => {
	...
	const highRelevanceDocs = documents
		.filter((doc: any) => doc.relevance === "high")
		.slice(0, 2);
	if (highRelevanceDocs.length > 0) {
		contextParts.push("  <detailed_excerpts>");
		highRelevanceDocs.forEach((doc: any) => {
