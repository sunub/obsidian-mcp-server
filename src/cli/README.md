...
## 이 디렉터리를 소개할 때 핵심적으로 말해야 할 것

이 CLI는 다음과 같이 소개하는 것이 가장 정확합니다.

> Obsidian Vault와 MCP 도구를 로컬 LLM 위에 연결하는 터미널 기반 AI 에이전트 UI.

즉, `src/cli`는 단순한 프론트엔드가 아니라:

- 사용자의 입력을 안전하게 받아들이고,
- 관련 문맥을 Vault에서 수집하고,
- 필요 시 MCP 도구를 호출하고,
- 그 결과를 스트리밍 응답으로 조합해 보여주는

**실행형 agent interface** 입니다.
