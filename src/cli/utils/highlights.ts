...
import { PASTED_TEXT_PLACEHOLDER_REGEX } from "../key/textBuffer/constants.js";
import type { Transformation } from "../key/textBuffer/types.js";
import { cpLen, cpSlice } from "./textUtil.js";
...
export function parseInputForHighlighting(
	...
	const parseUntransformedInput = (text: string): HighlightToken[] => {
		...
		match = HIGHLIGHT_REGEX.exec(text);
		while (match !== null) {
	...
	for (const transformation of sortedTransformations) {
		...
		tokens.push({
			...
			type: transformation.type === "paste" ? "paste" : "file",
		});
